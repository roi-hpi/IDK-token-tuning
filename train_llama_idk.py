...
from transformers import AutoModelForCausalLM, AutoTokenizer, MistralForCausalLM, PreTrainedModel
...
def main(fabric: L.Fabric, args: Args):
    ...
    with init_device:
        ...
        # Mistral not yet supported by PyTorch native FA2 instead (default fallback)
        attn_impl = "eager" if args.compile else "flash_attention_2"
        ...
        if args.use_additional_flash_attn_kernels and not args.compile:
            ...
            prevMistralRMSNorm = hf_mistral.MistralRMSNorm
            hf_mistral.MistralRMSNorm = FlashRMSNorm
        ...
        if args.use_additional_flash_attn_kernels and not args.compile:
            ...
            hf_mistral.MistralRMSNorm = prevMistralRMSNorm  # undo monkeypatch after model creation
    ...
    if fabric.is_global_zero:
        ...
        printer.debug("model num layers", len(model.model.layers), rank0_only=False)
        printer.debug("model hidden size", model.model.embed_tokens.weight.shape[-1], rank0_only=False)
    ...
    # if args.compile:
    #     13/01/24: compile is bugged w/ Mistral FA2 => don't use
...
def ce_loss_vector_target_correct_pred_reg(logits, targets):
    ...
    p_IDK_token = sm_logits[:, 32000]  # shape (num_masked_tokens)
...
def train(
    ...
    for i in step_bar:
            ...
            with fabric.no_backward_sync(model, enabled=not do_optimizer_step):
                ...
                if args.loss_type == "idk":
                    ...
                    with torch.no_grad():
                        ...
                        gold_probs = probs[is_target_not_minus_one].gather(dim=-1, index=hotfixed_targets.unsqueeze(-1)).squeeze(-1)
                if fabric.is_global_zero:
        ...
        if state["step_count"] % args.model_profiling_interval == 0 and args.model_profiling:
