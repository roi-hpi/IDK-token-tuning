...
############################################################
######### Install dependencies in seperate image ###########
############################################################
...
# RUN micromamba run -n base pip install ninja

# Need to build here with access to nvcc inside docker. 
RUN MAX_JOBS=20 micromamba run -n base pip install 'flash-attn>=2.3' --no-build-isolation
# RUN TORCH_CUDA_ARCH_LIST="8.0" MAX_JOBS=16 micromamba run -n base pip install 'flash-attn>=2.0.0.post1' --no-build-isolation

# Install RMSNorm kernel
USER root
RUN mkdir /flash-attn-norm
RUN chown -R $MAMBA_USER:$MAMBA_USER /flash-attn-norm 
USER $MAMBA_USER
RUN git clone https://github.com/Dao-AILab/flash-attention.git /flash-attn-norm
RUN cd /flash-attn-norm/csrc/layer_norm/ && MAX_JOBS=20 micromamba run -n base pip install .

RUN micromamba run -n base pip install entmax wechsel deepfocus
# Install from latest git for full FSDP support
RUN micromamba run -n base pip install git+https://github.com/Lightning-AI/lightning@532c723c8584903dc719458d0ad52861d51bc395
...
RUN echo "alias umamba=micromamba" >> /usr/local/bin/_activate_current_env.sh
